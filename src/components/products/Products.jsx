...
const Products = ({ data, isAdmin, style: { parent, child }, title, type }) => {
    ...
    <div key={product.id} className={child}>
      <div className="h-[349px] max-[600px]:h-[203px] relative overflow-hidden product-image">
        <img
          ...
          alt=""
        />
        <span className="absolute top-4 left-4 font-medium py-1 px-[14px] rounded shadow bg-white" type={type.name}>
          {type.name}
        </span>
        {isAdmin ? (
          <button
